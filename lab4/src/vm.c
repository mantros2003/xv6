...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  for(; a < newsz; a += PGSIZE){
    ...
    if(mem == 0){
      ...
      deallocuvm(pgdir, newsz, oldsz, myproc());
      return 0;
    }
    memset(mem, 0, PGSIZE);
...
void clear_proc_access_bits(struct proc *p) {
  pde_t *pde = p -> pgdir;

  static int i = 0, j = 0;
  int cleared = 0;
  for (int a = 0; a < NPDENTRIES; a++) {
    i = (i + a) % NPDENTRIES;
    if (pde[i] & PTE_P) {
      pte_t* pte = (pte_t*) P2V(PTE_ADDR(pde[i]));
      for (int b = 0; b < NPTENTRIES; b++) {
        j = (j + b) % NPTENTRIES;
        if (pte[j] & PTE_P && (pte[j] & PTE_A) && (pte[j] & PTE_U)) {
          pte[j] &= ~PTE_A;
          cleared++;
          if (cleared == 10) {
            return;
